...
> 本仓库由 **`codex_xiufu`**（多厂商协议中枢、可视化仪表盘、动态 Key 注入）与 **`codex_ture`**（Codex Desktop Electron 客户端底层加固、`features.js_repl` 防篡改、救急手册）两大核心工程深度融合而成。
> 解决 Codex 在本地使用第三方模型（Kimi、Gemini、MiniMax）及内置工具（Node REPL、浏览器控制、Tool Search）时的全部已知协议阻断与底层冲突。
...
|  * 多渠道模型轮换: Kimi k3 / Gemini Antigravity / MiniMax-M3             |
|  * 可视化控制台: http://127.0.0.1:15731/dashboard (Token 监控、测活、设置)  |
+--------------+---------------------+---------------------+--------------+
               |                     |                     |
               v                     v                     v
...
* **全流程协议改写与自学习**：完整修复 `tool_search` 孤儿调用、Schema `$ref` 递归内联、Namespace 动态学习。

### 二、客户端底层加固系统 (`client_patches/`)
* **配置防篡改 Patch**：解决 Codex Desktop 在同步时强行通过 `config/batchWrite` 将 `features.js_repl` 改写为 `false` 的问题。
* **安全沙盒原则**：原包只读，绝不修改 `C:\Program Files\WindowsApps` 下的源文件，全部修改位于 `%LOCALAPPDATA%\Codex-patched\<版本>` 副本。
* **自动化与幂等性**：支持版本自检、解包、定位混淆锚点、校验与重打包，带静态与动态校验。
* **救急恢复机制**：配套 [RESCUE.md](RESCUE.md) 救回手册，非本系统的任何 Agent 或人类均可在 30 秒内完整恢复。
...
│   ├── dashboard.py                   # 仪表盘 FastAPI 管理路由
│   ├── dashboard.html                 # 仪表盘管理页面（单页 SPA，零外部依赖，极客暗色 UI）
│   ├── bridge_stats.py                # Token 消耗与请求延迟 SQLite 异步采集
│   ├── bridge_config.example.json     # 多厂商配置模板（Kimi / Gemini / MiniMax）
│   └── assets/                        # 字体（Smiley Sans）、动效库（GSAP）与暗色状态图标
│
...
```
* **监控页**：查看请求总数、Token 用量与实时延迟。
* **设置页**：点击卡片可随时在 **Kimi For Coding**、**Gemini (Antigravity)** 与 **MiniMax 官方** 间免重启热切，并一键测活。
...
## 安全与免责声明
1. 本仓库严格遵守**敏感数据隔离原则**，示例配置（`bridge_config.example.json`）仅包含示例占位符，真实生产密钥与本地运行数据库均由 `.gitignore` 全面排除，绝不上云。
