...
# 1. 最小请求
r, err = post({"model": "gemini-3.7-flash-high", "stream": False, "input": [msg("say ok")]})
check("1 最小请求", r is not None and r.status == 200, err or "")

# 2. tool_search 工具定义转换
r, err = post({"model": "gemini-3.7-flash-high", "stream": False, "input": [msg("say ok")], "tools": [{"type": "tool_search"}]})
check("2 tool_search 工具定义转换", r is not None and r.status == 200, err or "")

# 3. 历史条目转换（tool_search_call / tool_search_output）
r, err = post({
    "model": "gemini-3.7-flash-high", "stream": False,
    "input": [
        msg("搜一下线程工具"),
...
r, err = post({
    "model": "gemini-3.7-flash-high", "stream": True,
...
r, err = post({
    "model": "gemini-3.7-flash-high", "stream": False, "input": [msg("say ok")],
